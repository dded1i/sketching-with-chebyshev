...
assert version.parse(torch.__version__.split('+')[0]) >= version.parse('1.8.0')
...
def gibbs_sampling_weighted_normal(input_dim, num_chains, gibbs_iterations=1, inv_resolution=1000):
    assert (num_chains > 0)
    """enerates a structured random matrix 𝑊"""
...
class AcosFeatureMap(nn.Module):
    """
    Implements the zeroth- and first-order arc-cosine feature maps, defined in Equation (11) of the paper.
    """
#corresponds to the arc-cosine features defined in Eq. (11).
...
class NtkFeatureMapOps(nn.Module):
    # implements the for-loop over layers recursively computing feature maps
    """
     parameter set up
    """
