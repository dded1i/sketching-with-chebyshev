...
def main():
    ...
    batch_size_candidates = [100, 50, 20, 10, 5, 2, 1]  # Skip huge batches
    ...
    z_tr = z[:x_tr.shape[0]].numpy()
    z_te = z[x_tr.shape[0]:].numpy()

    set_random_seed(args.seed)

    rand_idx = np.random.permutation(len(x_tr))
    train_fold = rand_idx[:100].tolist()
    valid_fold = rand_idx[100:200].tolist()
